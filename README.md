...
difficult part?  
For me it took 2 and half hours for the fully working solution. 1 hour for refactoring. Problem statement was easy but for certain senarios a little more explanation would have been great. 
...
If the batches are generated on the basis of some criteria like a batch will contain same interest rate etc. then we can tune our algorithm to process them faster as all of the loans will have same criteria. Batch processing might also decrease the performance if it is a randomly created batch. As there might be a loan for which interest rate might be high. In that case, we might have to check all the facilities before we hit the expected facility. Also, if the batch size is big the processing time might go up. 

6. Discuss your solution’s runtime complexity.
The time complexity will increase on the basis of number of facilites and covenants frequency. O(F*C).
